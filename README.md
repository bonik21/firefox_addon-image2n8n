...
웹페이지에서 우클릭한 이미지를 지정한 **n8n Webhook URL**로 즉시 전송해주는 Firefox 부가 기능(Add-on)입니다.
전송 방식은 Base64, Binary, URL을 지원합니다. 인증은 Basic Auth를 지원합니다.

사용자가 원하는 메뉴 명칭과 n8n 웹훅 URL 세트를 여러 개 등록하고 관리할 수 있어, 이미지 분석, 배경 제거, 단순 저장 등 작업 흐름에 따라 최적화된 n8n 워크플로우를 자유롭게 선택해 호출할 수 있습니다.

이 부가기능은 이미지를 전송하는 역할까지만 관여하며, 이미지를 받아서 처리하는 n8n워크플로우는 직접 구성해야 합니다. N8N을 사용하지 않을 경우 Python FastAPI등을 이용해 이미지를 처리하는 API서버를 직접 구성할 수도 있습니다.
